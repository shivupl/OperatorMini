# import json
# from gpt_parser import system_instructions, browser_instructions_from_context
    ...
    async with async_playwright() as p:
        ...
        while True:
            for step in actions:
                ...
                if curr_url != context.current_url or curr_title != context.current_title:
                    print(f"Detected new page: {curr_title} ({curr_url})")
                    dom_summary = await summarize_dom(page)
                    print("DOM SUMMARY DONE")
                    instructions = system_instructions(og_prompt, curr_url, curr_title, dom_summary, context.step_history)
                    next_json = browser_instructions_from_context(instructions, og_prompt, curr_url, curr_title, dom_summary, context.step_history)
                    try:
                        actions = json.loads(next_json)
                        context.update_state(curr_url, curr_title, dom_summary)
                        break  # restart with new actions
                    except Exception as e:
                        print("Failed to parse next actions:", e)
                        return
